...
//		urlAndMethodAndParameters= new String[]{"https://www.google.com","GET","q=test"};
		...
		if (urlAndMethodAndParameters == null) {
			urlAndMethodAndParameters = new String[]{};
		}
		if (urlAndMethodAndParameters.length > 0 && urlAndMethodAndParameters[0] != null) {
			u3[0] = urlAndMethodAndParameters[0];
		}
		if (urlAndMethodAndParameters.length > 1 && urlAndMethodAndParameters[1] != null) {
			u3[1] = urlAndMethodAndParameters[1];
		}
		if (urlAndMethodAndParameters.length > 2 && urlAndMethodAndParameters[2] != null) {
			u3[2] = urlAndMethodAndParameters[2];
		}
	...
	public static JSONObject fetchURL(String url, String method, String parameterString) throws Exception {
		...
		if (s.startsWith("[")) {
			rtn = new JSONObject("{\"arrayValue\":" + s + "}");
		} else if (s.length() > 0) {
			rtn.put("responseBody", s);
		}
		return rtn;
	}
